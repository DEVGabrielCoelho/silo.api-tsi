...
public class SecurityConfig {
	...
	private static final String[] WHITE_LIST_URL = { "/api/v1/management/**", "/v2/api-docs", "/v3/api-docs", "/v3/api-docs/**", "/swagger-resources", "/swagger-resources/**", "/configuration/ui", "/configuration/security", "/swagger-ui/**", "/webjars/**", "/swagger-ui.html" };
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		return http.csrf(csrf -> csrf.disable()).sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS)).exceptionHandling(ex -> ex.accessDeniedHandler(new CustomAccessDeniedHandler()))
				.authorizeHttpRequests(requests -> requests.requestMatchers(WHITE_LIST_URL).permitAll()
						.requestMatchers(HttpMethod.POST, WHITE_LIST_URL_TESTE).hasAnyAuthority("ADMIN", "USER")
						.requestMatchers(HttpMethod.GET, WHITE_LIST_URL_TESTE).hasAuthority("ADMIN")
						.requestMatchers(HttpMethod.PUT, WHITE_LIST_URL_TESTE).hasAuthority("ADMIN")
						.requestMatchers(HttpMethod.DELETE, WHITE_LIST_URL_TESTE).hasAuthority("ADMIN")
